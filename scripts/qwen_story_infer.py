...
def build_generation_record(
    ...
    return {
        "prompt_id": row["id"],
        "case_id": row.get("case_id", row["id"]),
        "scene_count": int(row.get("scene_count", row.get("num_panels", 0))),
        "num_panels": int(row.get("num_panels", row.get("scene_count", 0))),
        "layout": row.get("layout", "vertical"),
        "candidate": candidate,
        "seed": seed,
        "output_path": output_path.as_posix(),
        "prompt": row["prompt"],
...
def write_manifest(path: Path, records: list[dict[str, Any]]) -> None:
    ...
    payload = {
        "manifest_version": "qwen-story-page-v1",
...
def main() -> int:
        ...
        for row_index, row in enumerate(rows):
            for candidate in range(args.num_candidates):
                ...
                generated_records.append(
                    build_generation_record(row, candidate, seed, Path(result["output_path"]))
                )
                print(
    ...
    if args.manifest_out is not None:
        write_manifest(args.manifest_out, generated_records)
